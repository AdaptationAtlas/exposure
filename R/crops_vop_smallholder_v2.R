...
for(i in 1:length(Vop_Files)){
  Vop_data<-suppressWarnings(foreign::read.dbf(Vop_Files[i], as.is = T))
  ...
  for(j in 1:length(Cols)){
    ...
    names(crop_production)<-paste0(CROP,"-",FS,"-kg")
    names(crop_yield)<-paste0(CROP,"-",FS,"-kg_ha")
    ...
    plot(c(crop(crop_yield,ADMIN0),
           crop(crop_ha,ADMIN0),
           crop(crop_production,ADMIN0)))
